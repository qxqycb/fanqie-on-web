...
import random
from vercel_serverless import VercelServerless
...
def clear_cache():
    """Clear downloaded JSON files from cache"""
    try:
        # 清理bookstore目录中的JSON文件
        files_removed = 0
        for filename in os.listdir(BOOKSTORE_DIR):
            if filename.endswith('.json'):
                file_path = os.path.join(BOOKSTORE_DIR, filename)
                try:
                    os.remove(file_path)
                    files_removed += 1
                except Exception as e:
        ...
        return jsonify({
            'status': 'success',
            'message': f'已清理 {files_removed} 个缓存文件'
        })
    except Exception as e:
    ...
    try:
        ...
        for item in results:
            if 'book_data' in item and item['book_data']:
                ...
                books.append({
                    'bookId': str(book['book_id']),  # 确保ID是字符串
                    'bookName': book['book_name'],
                    'author': book['author'],
                    'wordCount': book['word_number'],
                    'desc': book.get('abstract', ''),  # 书籍简介
                    'coverUrl': book.get('thumb_url', ''),  # 封面URL
                    'status': '连载' if book.get('is_complete') == 0 else '完结'
                })
...
def reader_book_info(book_id):
    """获取书籍详情"""
    try:
        name, chapters, status = downloader._get_chapter_list(book_id)
        if name == 'err':
        ...
        for title, chapter_id in chapters.items():
            chapter_list.append({
                'title': title,
                'url': f'/api/v1/book/{book_id}/chapter/{chapter_id}'
            })
        ...
        logger.error(f"获取书籍信息出错: {str(e)}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/v1/book/<book_id>/chapter/<chapter_id>')
def reader_chapter_content(book_id, chapter_id):
    """获取章节内容"""
    try:
        # 获取章节内容
        ...
        # 解码内容
        decoded_content = downloader._decode_content(content)
        
        # 格式化章节内容，确保段落之间有适当的换行
        paragraphs = [p.strip() for p in decoded_content.split('\n') if p.strip()]
        formatted_content = '\n\n'.join(paragraphs)
    ...
    return jsonify({
        ...
        'apis': {
            ...
            'book_info': {
                ...
                'method': 'GET',
                'description': '获取书籍信息和章节列表'
            },
            'chapter_content': {
                'url': '/api/v1/book/{book_id}/chapter/{chapter_id}',
                'method': 'GET',
                'description': '获取章节内容'
            }
        }
...
# 修改书籍详情API
@app.route('/api/mobile/book/<book_id>')
def mobile_book_info(book_id):
    """获取书籍详情"""
    try:
        name, chapters, status = downloader._get_chapter_list(book_id)
        if name == 'err':
        ...
        return jsonify({
            ...
            'author': '',  # 如果能获取作者信息就填充
            'status': status[0] if status else '未知',
            'description': '',  # 如果能获取简介就填充
            'cover_url': '',    # 如果能获取封面就填充
            'chapters': chapter_list
        })
    except Exception as e:
        logger.error(f"获取书籍信息出错: {str(e)}")
...
# 修改章节内容API
@app.route('/api/mobile/chapter/<book_id>/<chapter_id>')
def mobile_chapter_content(book_id, chapter_id):
    """获取章节内容"""
    try:
        # 获取章节内容
        content = downloader._download_chapter_content(chapter_id)
        ...
        for category_name, keywords in category_keywords.items():
            ...
            for keyword in keywords:
                try:
                    results = downloader.search_novel(keyword)
                    if results:
                        for item in results[:2]:  # 每个关键词取2本书
                            if 'book_data' in item and item['book_data']:
                                book = item['book_data'][0]
                                # 避免重复添加
                                book_data = {
                                ...
                                if not any(b['id'] == book_data['id'] for b in all_books):
